...
def run_simulation(params: InputParams) -> SimulationResults:
    ...
    # per-tenor cashflow and loss trackers
    ...
    # compute true per-period cashflow components
    per_period_defaults = np.array([
        sum(default_by_tenor[n][t] for n in TERMS) for t in months
    ])
    per_period_interest  = np.array([
        sum(interest_by_tenor[n][t] for n in TERMS) for t in months
    ])
    per_period_principal = np.array([
        sum(principal_by_tenor[n][t] for n in TERMS) for t in months
    ])
    ...
    return SimulationResults(
        ...
        out_net=out_net,
        def_out=def_out,
        per_period_defaults=per_period_defaults,
...
def build_figures(params: InputParams, sim: SimulationResults) -> Dict[str, go.Figure]:
    ...
    # outstanding by tenor with optional default overlay
    ...
    net_moic   = {}
    for n in TERMS:
        gross_moic[n] = (1 + params.monthly_interest) ** n if n > 1 else 1 + params.monthly_interest
...
def render_dashboard(params: InputParams, sim: SimulationResults, figs: Dict[str, go.Figure]) -> None:
    ...
    with st.expander("Breakdown of defaults, interest, reinvestment", expanded=False):
        ...
        st.plotly_chart(px.area(df_metrics, x='Month', y=['Defaults', 'Interest', 'Reinvested']), use_container_width=True)
    ...
    with st.expander("See cumulative build-up", expanded=False):
        ...
        st.plotly_chart(px.area(df_cum, x='Month', y=['Cum Defaults', 'Cum Interest', 'Cum Reinvested']), use_container_width=True)
