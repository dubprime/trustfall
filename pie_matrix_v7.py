...
def load_inputs() -> InputParams:
    ...
    overlay = st.sidebar.checkbox("Overlay defaults within gross view", value=True)
...
def run_simulation(params: InputParams) -> SimulationResults:
    ...
    per_prin     = np.array([sum(principal_by_tenor[n][t] for n in TERMS) for t in months])
...
def build_figures(params: InputParams, sim: SimulationResults) -> Dict[str, go.Figure]:
    ...
    # Outstanding by tenor + defaults overlay
    ...
    fig_ost.add_trace(go.Scatter(
        x=sim.months, y=total, name="Total", mode="lines+markers",
    ...
    figs["defaults"] = px.bar(
        df_def, x="Month", y=[f"{n}-Month Default" for n in TERMS],
    ...
    figs["interest"] = px.bar(
        df_int, x="Month", y=[f"{n}-Month Interest" for n in TERMS],
    ...
    figs["principal"] = px.bar(
        df_prin, x="Month", y=[f"{n}-Month Principal" for n in TERMS],
    ...
    for n in TERMS:
        fig_net.add_trace(go.Scatter(
            x=sim.months, y=sim.out_net[n], name=f"{n}-Month Net",
            mode="lines+markers", line=dict(color=COLORS[n])
        ))
    fig_net.update_layout(xaxis_title="Month", yaxis_title="Net Outstanding ($)")
    figs["net_out"] = fig_net

    # Yield curve (Gross vs Net MOIC)
    deployed = {n: sim.new_by_t[:,i].sum() for i,n in enumerate(TERMS)}
    gross_moic = {n: (1+params.monthly_interest)**n if n>1 else 1+params.monthly_interest for n in TERMS}
    net_moic = {n: sim.payment_by_tenor[n].sum()/deployed[n] if deployed[n]>0 else 0 for n in TERMS}
    ...
    figs["yield_curve"] = px.bar(
        df_yld, x="Tenor", y=["Gross MOIC","Net MOIC"],
...
def render_dashboard(params: InputParams, sim: SimulationResults, figs: Dict[str, go.Figure]) -> None:
    ...
    with st.expander("Breakdown of defaults, interest, reinvestment", expanded=False):
        df_metrics = pd.DataFrame({
            'Month': sim.months + 1,
            'Defaults': sim.per_period_defaults,
            'Interest': sim.per_period_interest,
            'Reinvested': sim.per_period_principal,
        })
        st.dataframe(df_metrics.set_index('Month').style.format("{:,.2f}"))
        st.plotly_chart(px.area(
            df_metrics, x="Month", y=["Defaults","Interest","Reinvested"],
            color_discrete_sequence=list(COLORS.values())
    ...
    with st.expander("See cumulative build-up", expanded=False):
        ...
        st.plotly_chart(px.area(
            df_cum, x="Month", y=["Cum Defaults","Cum Interest","Cum Reinvested"],
            color_discrete_sequence=list(COLORS.values())
        ), use_container_width=True)
